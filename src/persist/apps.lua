...
function apps.normalize_modules(requested)
    ...
    -- Имя своего композитора она читает модулем `ctx` — но его объявляет она
    -- сама: библиотека получает СВОИ модули, а не модули окна, и расширять
    -- ради этого белый список окна не нужно.
    add("process")
    table.sort(out)
    return out
end
